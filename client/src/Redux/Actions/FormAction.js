...
export const formDetails = (data1) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(`https://medicalnetworkapp.herokuapp.com/api/form/postform`,data1);
...
export const listFormsbyEmail = (email) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`https://medicalnetworkapp.herokuapp.com/api/form/getbyemail/${email}`);
