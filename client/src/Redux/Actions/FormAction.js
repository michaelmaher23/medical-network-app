...
export const formDetails = (data1) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(`https://medicalprojectnet.vercel.app/api/form/postform`,data1);
...
export const listFormsbyEmail = (email) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`https://medicalprojectnet.vercel.app/api/form/getbyemail/${email}`);
