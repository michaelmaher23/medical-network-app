...
userRouter.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (user && (await user.matchPassword(password))) {
      ...
      const refresh_token = generatRefreshToken({ id: user._id }, res);
      res.cookie("refreshtoken", refresh_token, {
        maxAge: 30 * 24 * 60 * 60 * 1000, // 30 day
      });
      const user1=User.findByIdAndUpdate(
        { _id: user._id },
        {
...
userRouter.post("/social", async (req, res) => {
  ...
  try {
    ...
    if (userExists) {
      const access_token = generateAccessToken({ id: userExists._id });
      const refresh_token = generatRefreshToken({ id: userExists._id }, res);
      res.cookie("refreshtoken", refresh_token, {
        maxAge: 30 * 24 * 60 * 60 * 1000, // 30 day
      });
...
style="font-family:Poppins,Arial,Helvetica,sans-serif;font-size:15px;<a>
<!-- End --></body></html>`,
...
userRouter.get("/reftoken", async (req, res) => {
  try{
...
console.log(`${user.rf_token}`)
  const access_token = generateAccessToken({ id: user._id });
  const refresh_token = generatRefreshToken({ id: user._id }, res);
  console.log(`iiiiii${refresh_token}`);
